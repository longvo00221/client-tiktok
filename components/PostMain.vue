  ...
  <div  :id="`PostMain-${post.id}`" class="flex border-b py-6">
    ...
    <div class="pl-3 w-full px-4">
      <div class="flex items-center justify-between pb-0.5">
        <button @click="$e=> isLoggedIn(post.user)" class="font-bold hover:underline cursor-pointer">
      ...
      <div class="text-[14px] text-gray-500 pb-0.5">#xh #Fyp #Foryou</div>
      ...
      <div class="mt-2.5 flex">
        ...
        <div class="relative mr-[75px]">
            <div class="absolute bottom-0 pl-7">
                <div class="pb-4 text-center">
                    <button
                    @click="$e => isLiked ? unlikePost(post) : likePost(post)"
                ...
                <div class="pb-4 text-center">
                    ...
                    <span class="text-xs text-gray-800 font-semibold">230K</span>
                </div>
                <div class="pb-4 text-center">
                    <button class="rounded-full bg-gray-200 p-2 cursor-pointer">
                        <Icon name="ri:share-forward-fill" size="25"/>
                    </button>
                    <span class="text-xs text-gray-800 font-semibold">2K</span>
                </div>
            </div>
        </div>
      </div>
...
onMounted(() => {
  let observer = new IntersectionObserver(function(entries) {
    if(entries[0].isIntersecting){
      ...
      console.log('Element is not playing' + post.value.id);
      // video.value.pause()
    }
  },{threshold:[0.6]})
