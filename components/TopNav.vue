  ...
  >
    ...
    >
      <div :class="route.fullPath === '/' ? 'w-[80%]' : 'lg:w-[20%] w-[70%]'">
        <NuxtLink to="/">
          <img src="~/assets/images/tiktok-logo.png" width="115" alt="logo" />
        </NuxtLink>
      </div>
      <div
      ...
      >
        <input
          type="text"
          class="w-full px-3 my-2 bg-transparent placeholder-[#838383] text-[15px] focus:outline-none"
      ...
      >
        <button
          @click="$e=>isLoggedIn()"
          class="flex items-center border rounded-lg px-3 py-[6px] hover:bg-gray-100"
        >
        ...
        <div v-if="!$userStore.id" class="flex items-center">
          ...
          @click="$event => $generalStore.isLoginOpen = true"
            class="flex items-center bg-[#F02C56] text-white border rounded-lg px-3 py-[6px]"
          >
        ...
        <div v-else class="flex items-center">
          ...
          <div class="relative">
            ...
            >
              ...
              <button @click="$event => logout()" class="flex w-full items-center justify-start py-3 px-1.5 hover:bg-gray-100 border-t cursor-pointer">
                <Icon
